...
void pmm_init_bitmap(void* address, size_t size){
    bitmap_init(address, size, &pmm_bitmap);
}

void* pmm_allocate_page(void){
    atomic_acquire(&pmm_lock);

    for(uint64_t index = pmm_first_free_page_index; index < pmm_memory_info.total_page_count; index++){
        if(!bitmap_get_set_bit(&pmm_bitmap, index, true)){
            pmm_first_free_page_index = index;
            pmm_memory_info.free_page_count--;
            pmm_memory_info.used_page_count++;
    ...
    // panic
    kernel_panic("not enough memory available");
    atomic_release(&pmm_lock);
...
void* pmm_allocate_pages(uint64_t page_count){
	...
	while(pmm_free_list->page_count < page_count){
        if(pmm_free_list->last != NULL){
            ...
            kernel_panic("not pmm_lock memory available");
            atomic_release(&pmm_lock);
	        return NULL;
        }
    }
...
uint64_t pmm_get_total_memory_size(void){
    return pmm_memory_info.total_page_count * PAGE_SIZE;
}

uint64_t pmm_get_free_memory_size(void){
    return pmm_memory_info.free_page_count * PAGE_SIZE;
}

uint64_t pmm_get_used_memory_size(void){
    return pmm_memory_info.used_page_count * PAGE_SIZE;
}

uint64_t pmm_get_reserved_memory_size(void){
    return pmm_memory_info.reserved_page_count * PAGE_SIZE;
}
